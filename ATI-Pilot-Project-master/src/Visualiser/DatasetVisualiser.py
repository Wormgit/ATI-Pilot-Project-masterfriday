...
class DatasetVisualiser(object):
    ...
    def visualiseObjectSizeDistribution(anno_path):
        ...
        for size in obj_sizes:
            ...
            if fn >= indoor0[0] and fn <= indoor0[1]:
                indoor_sizes.append(size['size'])
            elif fn >= outdoor0[0] and fn <= outdoor0[1]:
                outdoor0_sizes.append(size['size'])
            elif fn >= indoor1[0] and fn <= indoor1[1]:
                indoor_sizes.append(size['size'])
            elif fn >= outdoor1[0] and fn <= outdoor1[1]:
                outdoor1_sizes.append(size['size'])
    ...
    def visualiseInstancesPerDay(splits_filepath, num_classes):
        ...
        plt.xlim((-0.5, num_classes + 11))
        plt.ylim((0.01, np.max(bar_sum) + 5))
        plt.xticks(ind, tuple(str(x).zfill(3) for x in ind.tolist()), rotation="vertical", fontsize=14)  #
...
if __name__ == '__main__':
    ...
    # can run
    # splits_dir = "/home/io18230/Desktop/RGBDCows2020/Identification/single_train_valid_test_splits.json"
