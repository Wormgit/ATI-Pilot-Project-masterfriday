...
def evaluateModel_allpkl(args, num_classes=172, softmax_enabled=1, ckpt_path ='', save_name=''):
    ...
    if args.test_folder_class == 3: # we only need embeddings
        f_embeddings, _, name = inferEmbedding_from_folder_3(args, model)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_num', type=int,
                        default=156)#)40)  # need to fill in one that is the same as the number of trainng folders if using softmax. not applicable
    # flexible
