...
def knn_correct(name, f_preds):
    # now it works for 2-class folders.
    correct_pre = []
...
def evaluateModel(args, num_classes=186, softmax_enabled=1):
    ...
    if len(correct_pre) > 0 :
        print(f"KNN accuracy of the test folder={str(round(f_accuracy, 2))} % \n")
        data1 = pd.DataFrame({'image_path': name, 'pre_label_knn': f_preds, 'corr_label_knn': correct_pre, 'model_label': f_labels})
        woduplicates = set(correct_pre) #number of tested ID
        data1.to_csv(args.save_path +'/'+dateinfo+'/1Will/label_{}_'.format(str(len(woduplicates)))+str(len(set(f_labels)))+'.csv')
    else:
        print(f"3 class, no KNN accuracy")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_dir', nargs='?', default="/home/io18230/Desktop/make_all_data/Crop_split/0225")
    #parser.add_argument('--test_dir', nargs='?', default="/home/io18230/Desktop/Crop_split3-10_3-11") #RGB2  sub_will
    #parser.add_argument('--test_dir', nargs='?', default="/home/io18230/Desktop/RGBDCows2020w/test_2class_4days_exclude_noblack/") # RGBDCows2020_
    ...
    if os.path.exists("/home/io18230/Desktop"):  # Home machine
        ...
        embeddings_file = pt + '/train_embeddings.npz' # it need help from the training embedddings
    else:
        if os.path.exists("/home/will"):  # GPU machine
