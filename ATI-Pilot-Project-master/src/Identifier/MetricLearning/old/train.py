...
def trainFold(args):
	...
	for epoch in tqdm(range(args.num_epochs), desc="Training epochs"):
		...
		for images, images_pos, images_neg, labels, labels_neg, sub_p, sub_n, random_mark in data_loader:
			...
			with torch.no_grad():
				val_losses = []
				t_data_loader, model, _, _ = utils.setupForTraining(args=args, split ='valid')
				...
				valLoss_All_mean.append(round(valLoss_All_mean, 4))
				np.savez(os.path.join(args.fold_out_path, "valLoss_All_mean.npz"), valLoss_All_mean=valLoss_All_mean)
