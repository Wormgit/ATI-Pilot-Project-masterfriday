...
parser.add_argument('--frame_file', default='/home/io18230/Desktop/Identification_WILL155/RGB (copy)', type=str)
...
def make_train_data(items,it):
    dest = args.frame_file+'/'+ str(it).zfill(4)
    #makedirs(dest)
    src = os.path.join(args.frame_file + '/' + items)
    #shutil.copy(os.path.join(args.frame_file+'/'+ items), os.path.join(dest))
    os.rename(src, dest)
