...
def trainFold(args):
	...
	valLoss_1st_single = []
	TrainLoss_1stStepN = []
	TrainLoss_1st_single=[]
	if args.collect_1st:
		utils.saveCheckpoint(global_step, model, optimiser, "Firststeps{}".format("%03d" % global_step))
		with torch.no_grad():
			val_losses = []
			for images, images_pos, images_neg, labels, _, sub_p, _, _ in t_data_loader:
				...
				val_loss = loss_fn(embed_anch, embed_pos, embed_neg, labels, sub_p)
				val_losses.append(val_loss.item())
			valLoss_1st_single.append(round(np.mean(val_losses), 4))
			valLoss_1stStepN.append(global_step)
			np.savez(os.path.join(args.fold_out_path, "valLoss_1st.npz"),
					 valLoss_1stStepN=valLoss_1stStepN, valLoss_1st_single=valLoss_1st_single)

	# Main training loop
	for epoch in tqdm(range(args.num_epochs), desc="Training epochs"):
		# Mini-batch training loop over the training set
		for images, images_pos, images_neg, labels, labels_neg, sub_p, sub_n, random_mark in data_loader:
		###for images, images_pos, images_neg, labels, labels_neg in data_loader:
			...
			if "Softmax" in args.loss_function:
				...
				loss = loss_fn(embed_anch, embed_pos, embed_neg, labels, sub_p)
			...
			if args.collect_1st:
				if global_step == 1 or global_step == 10 or global_step == 100:
					utils.saveCheckpoint(global_step, model, optimiser, "Firststeps{}".format("%03d" % global_step))
					TrainLoss_1stStepN.append(global_step)
					TrainLoss_1st_single.append(loss.item())
					np.savez(os.path.join(args.fold_out_path, "TrainLoss_1st.npz"),
							 TrainLoss_1stStepN=TrainLoss_1stStepN, TrainLoss_1st_single=TrainLoss_1st_single)
					with torch.no_grad():
						val_losses = []
						for images, images_pos, images_neg, labels, _, sub_p, _, _ in t_data_loader:
							if not os.path.exists("/home/io18230/Desktop"):
		...
		if epoch % args.eval_freq == 0:
			...
			if args.save_best_or_each:
				...
				utils.saveCheckpoint(epoch, model, optimiser, "current{}".format("%03d" % epoch))
			else:
...
if __name__ == '__main__':
	...
	parser.add_argument('--out_path', type=str, default="/home/io18230/Desktop/metric_learning",
	...
	parser.add_argument('--model', type=str, default='TripletResnet',
	...
	parser.add_argument('--loss_function', type=str, default='OnlineTripletLoss',
	...
	parser.add_argument('--valLoss', type=int, default=1)
	parser.add_argument('--collect_1st', type=int, default=1)
