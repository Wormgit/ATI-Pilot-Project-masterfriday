...
#from Datasets.RGBDCows2020_will import RGBDCows2020_2
...
def Resize_pad_image(image, target_scale):
    iw, ih = image.size  # 原始图像的尺寸
    w, h = (int(iw*target_scale), int(ih*target_scale))  # target size
    nw = int(iw * target_scale)  # only if the image is square
    image = image.resize((nw, nw), PIL.Image.BICUBIC)  # resize
    new_image = PIL.Image.new('RGB', (iw, ih), (0,0,0))  # black around pic
    new_image.paste(image, ((iw - w) // 2, (ih - h) // 2))
    return new_image
...
class Augment:
    # If want to check image, show_to_check =1
    def __init__(self, n, show_to_check = 0):
        self.n = n
        self.show = show_to_check
    ...
    def __call__(self, img):
        ...
        for op, minval, maxval in ops:
            ...
            if self.show:
                img.show()
                sys.exit(1) # show and end the code.
        return img
...
class Utilities:
    ...
    def setupForTraining(self, args, split='train', transform = None):
        ...
        elif args.triplet_selection == "PositiveNegative":
            triplet_selector = HardestPositiveTripletSelector(margin=args.triplet_margin, marginP = args.Positive_margin)
