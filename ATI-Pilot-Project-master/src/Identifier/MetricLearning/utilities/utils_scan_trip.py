...
from Datasets.scan_data_trip import RGBDCows2020
...
EPS = 1e-8
def entropy(x, input_as_probabilities): # did not use
    """
...
class SCANLoss(nn.Module):           # did not use
    def __init__(self, entropy_weight=0.5):
        super(SCANLoss, self).__init__()
        self.softmax = nn.Softmax(dim=1)
        self.bce = nn.BCELoss()
        self.entropy_weight = entropy_weight  # Default = 2.0

    def forward(self, anchor_embed, neighbors_embed):
        """
        input:
        ...
        """
        # Softmax
        b, n = anchor_embed.size()
        anchors_prob = self.softmax(anchor_embed)
        positives_prob = self.softmax(neighbors_embed)
    ...
    def selectDataset(args, split, transform):
        ...
        if args.dataset == "OpenSetCows2020":
            dataset = OpenSetCows2019(args.unknown_ratio,
                                      ...
                                      combine=True,
                                      suppress_info=False)
        elif args.dataset == "RGBDCows2020" or args.dataset == "scan_data":
