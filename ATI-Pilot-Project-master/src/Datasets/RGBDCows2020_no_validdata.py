...
class RGBDCows2020(data.Dataset):
		...
		for current_RGB, current_D in zip(self.__RGB_folders, self.__D_folders):
			...
			for inter in os.listdir(current_RGB):
				...
				if self.__split_mode == "trainvalidtest":
					...
					tmp_test[inter]  = list(self.__splits[raw_ID][inter]['valid'])
					tmp_val[inter]   = list(self.__splits[raw_ID][inter]['test'])
			...
			self.__sorted['valid'][raw_ID][inter] =  tmp_val
			self.__sorted['test'][raw_ID][inter] = tmp_test
	...
	def __getitem__(self, index):
		...
		# Otherwise we're in triplet mode
			...
			img_neg, label_neg, sub_folder, random_mark = self.__retrieveNegative(label_anchor_orig,sub_id_orig)

			# Convert label to numpy
			label_neg = np.array([int(label_neg)])
			sub_folder= np.array([int(sub_folder)]) ###
	...
	def __retrievePositive(self, anchor_filename, label_anchor,sub):
		...
		filenames = list(self.__sorted["train"][label_anchor][sub])
	...
	def __retrieveNegative(self, label_anchor, sub_id):
		...
		sub_track = list(self.__sorted["train"][label_anchor].keys())
		if random.randint(1,10) >=6 and len(sub_track) > 1:
			...
			random_neg = list(self.__sorted["train"][category_seltec].keys())
